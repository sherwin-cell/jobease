...
<div class="job-detail-wrapper">
    ...
    <div class="job-card">
        ...
        @php
            ...
            // Clean and normalize candidate skills (handle array or string)
            $candidateSkills = collect();
            if ($candidate && $candidate->skills) {
                if (is_string($candidate->skills)) {
                    $candidateSkills = collect(explode(',', $candidate->skills))
                        ->map(fn($s) => strtolower(trim($s)))
                        ->filter()
                        ->unique();
                } elseif (is_array($candidate->skills)) {
            ...
            // Clean and normalize job required skills (handle array or string)
            $jobSkills = collect();
            if ($job->skills_required) {
                if (is_string($job->skills_required)) {
                    $jobSkills = collect(explode(',', $job->skills_required))
                        ->map(fn($s) => strtolower(trim($s)))
                        ->filter();
                } elseif (is_array($job->skills_required)) {
                    $jobSkills = collect($job->skills_required)
                        ->map(fn($s) => strtolower(trim($s)))
                        ->filter();
                }
            } elseif ($job->description) {
                // Fallback: extract from description if no skills defined
                $commonSkills = ['laravel', 'php', 'javascript', 'react', 'vue', 'python', 'java', 'html', 'css', 'sql', 'mysql', 'postgresql', 'mongodb', 'aws', 'docker', 'git'];
                $jobSkills = collect();
            ...
            // Calculate matched skills using collection intersection
            $matchedCollection = $jobSkills->intersect($candidateSkills);
            $matched = $matchedCollection->count();
            $totalNeeded = $jobSkills->count();
            $match = $totalNeeded > 0 ? round(($matched / $totalNeeded) * 100) : 0;
        ...
        @else
            <div class="match-card">
                ...
                <div class="match-message">
                    <span style="font-size: 1.25rem;">{{ $matchIcon }}</span>
                    <span>{{ $matchMessage }}</span>
                </div>
                
                @if($matchedCollection->count() > 0)
                    <div style="margin-top: 16px;">
                        <p style="font-size: 0.75rem; font-weight: 600; color: #1e3a8a; margin-bottom: 8px;">
                            ✓ Your matched skills:
                        </p>
                        <div class="matched-skills-list">
                            @foreach($matchedCollection as $skill)
                                <span class="matched-skill">{{ ucfirst($skill) }}</span>
                            @endforeach
                        </div>
                    </div>
                @endif
                ...
                @if($missingSkills->count() > 0 && $match < 100)
                    <div style="margin-top: 12px;">
                        <p style="font-size: 0.75rem; font-weight: 600; color: #78350f; margin-bottom: 8px;">
                            ⚠ Skills to develop:
                        </p>
                        <div class="matched-skills-list">
                            @foreach($missingSkills->take(5) as $skill)
                                <span style="background: #fffbeb; color: #92400e; padding: 4px 12px; border-radius: 999px; font-size: 0.75rem;">
                                    {{ ucfirst($skill) }}
                                </span>
                            @endforeach
                            @if($missingSkills->count() > 5)
                                <span style="background: #fffbeb; color: #92400e; padding: 4px 12px; border-radius: 999px; font-size: 0.75rem;">
                                    +{{ $missingSkills->count() - 5 }} more
                                </span>
                            @endif
                        </div>
                    </div>
                ...
                @if($match == 100 && $totalNeeded > 0)
                    <div style="margin-top: 12px; padding: 8px 12px; background: #dcfce7; border-radius: 8px;">
        ...
        <div class="job-meta-grid">
            <div class="meta-item">
                ...
                <div>
                    ...
                    <p class="meta-value">{{ ucfirst($job->experience_level ?? 'Any level') }}</p>
                </div>
            </div>
            ...
            <div class="meta-item">
                ...
                <div>
                    ...
                    <p class="meta-value">
                        {{ $job->salary ? '$' . number_format($job->salary) . '/year' : 'Negotiable' }}
                    </p>
                </div>
            </div>
