    ...
    <div class="container mx-auto p-6">
        ...
        <form method="POST" action="{{ route('profile.update') }}">
            ...
            <div class="mb-4">
                ...
                @php
                    $selectedSkillIds = old('skills', $profile->skills?->pluck('id')?->all() ?? []);
                @endphp
                <select name="skills[]" multiple class="border p-2 w-full min-h-[120px]">
                    @foreach($skills as $skill)
                        <option value="{{ $skill->id }}" @if(in_array($skill->id, $selectedSkillIds)) selected @endif>
                            {{ $skill->name }}
                        </option>
                    @endforeach
                </select>
                <p class="text-sm text-gray-600 mt-1">Hold Ctrl (Windows) / Cmd (Mac) to select multiple skills.</p>
            </div>
            ...
            <div id="experiences" class="mb-4">
                    ...
                    @foreach($profile->experience as $i => $exp)
                        <div class="mb-2 border p-2 rounded">
                            <input type="text" name="experience[{{ $i }}][title]" value="{{ $exp['title'] ?? '' }}"
                                placeholder="Title" class="border p-1 w-full mb-1">
                            <input type="text" name="experience[{{ $i }}][company]" value="{{ $exp['company'] ?? '' }}"
                                placeholder="Company" class="border p-1 w-full mb-1">
                            ...
                            <textarea name="experience[{{ $i }}][description]" placeholder="Description"
                                class="border p-1 w-full mb-1">{{ $exp['description'] ?? '' }}</textarea>
                            <button type="button"
                ...
                @else
                    {{-- Empty placeholder --}}
                    <div class="mb-2 border p-2 rounded">
                        <input type="text" name="experience[0][title]" placeholder="Title" class="border p-1 w-full mb-1">
                        <input type="text" name="experience[0][company]" placeholder="Company" class="border p-1 w-full mb-1">
                        <input type="date" name="experience[0][start_date]" class="border p-1 w-full mb-1">
                        <input type="date" name="experience[0][end_date]" class="border p-1 w-full mb-1">
                        <textarea name="experience[0][description]" placeholder="Description"
                ...
                {{-- Add button --}}
                <button type="button" id="add-experience" class="bg-blue-500 text-white px-4 py-1 rounded mt-2">Add
                    Experience</button>
            </div>
            ...
            <div class="mb-4">
                    ...
                    @foreach($profile->education ?? [] as $i => $edu)
                        <div class="mb-2 border p-2 rounded">
                            <input type="text" name="education[{{ $i }}][degree]" value="{{ $edu['degree'] ?? '' }}"
                                placeholder="Degree" class="border p-1 w-full mb-1">
                            <input type="text" name="education[{{ $i }}][school]" value="{{ $edu['school'] ?? '' }}"
                                placeholder="School" class="border p-1 w-full mb-1">
                ...
                <button type="button" id="add-education" class="bg-blue-500 text-white px-4 py-1 rounded mt-2">Add
                    Education</button>
            </div>
            ...
            <div class="mb-4">
                ...
                <button type="button" id="add-cert" class="bg-blue-500 text-white px-4 py-1 rounded mt-2">Add
            ...
            <div class="mb-4">
                ...
                <button type="button" id="add-interest" class="bg-blue-500 text-white px-4 py-1 rounded mt-2">Add
                    Interest</button>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.getElementById('add-experience').addEventListener('click', function () {
                ...
                const count = wrapper.querySelectorAll('div.mb-2').length; // number of existing blocks
                const div = document.createElement('div');
                div.classList.add('mb-2', 'border', 'p-2', 'rounded');
                div.innerHTML = `
                        <input type="text" name="experience[${count}][title]" placeholder="Title" class="border p-1 w-full mb-1">
                        <input type="text" name="experience[${count}][company]" placeholder="Company" class="border p-1 w-full mb-1">
                        <input type="date" name="experience[${count}][start_date]" class="border p-1 w-full mb-1">
                        <input type="date" name="experience[${count}][end_date]" class="border p-1 w-full mb-1">
                        <textarea name="experience[${count}][description]" placeholder="Description" class="border p-1 w-full mb-1"></textarea>
                ...
                div.querySelector('.remove-experience').addEventListener('click', e => div.remove());
            });

            // Remove existing experience blocks
            document.querySelectorAll('.remove-experience').forEach(btn => {
                btn.addEventListener('click', e => e.target.closest('div.mb-2').remove());
            });
            // Education
            document.getElementById('add-education').addEventListener('click', function () {
                const wrapper = document.getElementById('education');
                const count = wrapper.querySelectorAll('div.mb-2').length; // existing blocks count
                const div = document.createElement('div');
                div.classList.add('mb-2', 'border', 'p-2', 'rounded');
                div.innerHTML = `
            <input type="text" name="education[${count}][degree]" placeholder="Degree" class="border p-1 w-full mb-1">
            <input type="text" name="education[${count}][school]" placeholder="School" class="border p-1 w-full mb-1">
            <input type="date" name="education[${count}][start_date]" class="border p-1 w-full mb-1">
            <input type="date" name="education[${count}][end_date]" class="border p-1 w-full mb-1">
            <button type="button" class="remove-education bg-red-500 text-white px-2 py-1 mt-1 rounded">Remove</button>
        `;
                // Append new education block to the wrapper
                wrapper.appendChild(div);

                // Add event listener for the remove button
                div.querySelector('.remove-education').addEventListener('click', function () {
                    div.remove();
                    // Optional: reindex names after removal
                    updateEducationIndices();
                });
            });

            // Remove existing education blocks
            document.querySelectorAll('.remove-education').forEach(btn => {
                btn.addEventListener('click', function () {
                    this.closest('div.mb-2').remove();
                    updateEducationIndices();
                });
            });

            // Optional: function to update indices to prevent duplicate names
            function updateEducationIndices() {
                const blocks = document.querySelectorAll('#education div.mb-2');
                blocks.forEach((block, index) => {
                    block.querySelectorAll('input').forEach(input => {
                        const nameParts = input.name.split(']');
                        input.name = `education[${index}]` + nameParts[1];
                    });
                });
            }
        });
