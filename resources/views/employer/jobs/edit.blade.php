...
@section('content')
    <div class="max-w-3xl mx-auto mt-8 bg-white p-6 rounded shadow">
        ...
        <form method="POST" action="{{ route('employer.jobs.update', $job) }}">
            ...
            @php
                $liveQa = $job->liveSkillQa;
                $live = old('live_skill_qa_enabled', (int) (($liveQa->enabled ?? false) ? 1 : 0));
                ...
                if (count($slotStartsOld) || count($slotEndsOld)) {
                    ...
                    $rows = ($liveQa->slots ?? []) ?: [['start_at' => null, 'end_at' => null]];
                }
            @endphp
                ...
                <div id="live-skill-qa-fields" class="mt-4 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <textarea name="live_skill_qa_question_categories" rows="4" class="w-full border rounded p-2">{{ old('live_skill_qa_question_categories', is_array($liveQa->question_categories ?? null) ? implode("\n", $liveQa->question_categories) : '') }}</textarea>
                        </div>
                        <div>
                            <label class="block font-medium mb-1">Specific questions (one per line)</label>
                            <textarea name="live_skill_qa_questions" rows="4" class="w-full border rounded p-2">{{ old('live_skill_qa_questions', is_array($liveQa->questions ?? null) ? implode("\n", array_map(fn($q) => $q['question'] ?? '', $liveQa->questions)) : '') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
